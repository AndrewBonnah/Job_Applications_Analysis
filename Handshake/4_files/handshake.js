  ...
  window.duet =
    ...
    (function () {
      ...
      function stripUrlParams(url) {
        // Use the URL API to parse the URL if it's a valid URL
        try {
          ...
          // If the URL is not valid or the URL API is not supported, fallback to a regex solution
          return url.split("?")[0];
        }
      }
      ...
      function getWidgetLocation() {
        ...
        return stripUrlParams(url);
      }

      function getBaseMetadata() {
        return {
          widget_location: getWidgetLocation(),
        };
      }
      ...
      return {
        init: function () {
          const loadWidget = () => {
            ...
            const widgetUrl = `https://decagon.ai/demo/handshake`;

            iframe.src = widgetUrl;
            iframe.title = "Chatbot Assistance and Contact Support";
            iframe.id = "decagon-iframe";

            document.body.appendChild(iframe);
            let minimizeTask = null;

            sendMessageSafely({ addMetadata: getBaseMetadata() });
            ...
            window.addEventListener("message", function (e) {
              ...
              if (data.type === "DECAGON_LOADED") {
                ...
                minimizeTask = this.setTimeout(() => {
                  iframe.style.width = data.isHidden ? "0px" : "350px";
                  iframe.style.height = data.isHidden ? "0px" : "200px";
